...
#include "../../external/manifoldReconstructor/include/OpenMvgParser.h"
...
void OpenMvgParser::parse() {
  ...
  //modified
/*  char buffer[65536];
  rapidjson::FileReadStream is(pFile_, buffer, sizeof(buffer));
  document_.ParseStream<0, rapidjson::UTF8<>, rapidjson::FileReadStream>(is);*/

	rapidjson::IStreamWrapper isw(fileStream_);
	document_.ParseStream(isw);

  try {
	    if (document_.IsNull())
	      throw JsonParseException("JsonParseException--> the json file " + fileName_ + " is NULL");
  ...
  parseExtrinsics(extrinsics,map_pos,cam_ids);

  parseViews(intrinsics, extrinsics,map_pos,cam_ids);

  parsePoints(map_pos);
  fileStream_.close();
}

void OpenMvgParser::parseViews(const std::map<int, glm::mat3> & intrinsics, const std::map<int, CameraType> & extrinsics, const std::map<int,int> &map_pos, const std::vector<int> &cam_ids) {
...
void OpenMvgParser::parsePoints(const std::map<int,int> &map_pos) {
  try {
    ...
    for (rapidjson::SizeType curPoint = 0; curPoint < structure.Size(); curPoint++) { // Uses SizeType instead of size_t
      ...
      for (rapidjson::SizeType curId = 0; curId < observations.Size(); curId++) {
        ...
        int curCam = map_pos.at(observations[curId]["key"].GetInt());
...
void OpenMvgParser::parseExtrinsics(std::map<int, CameraType> & extrinsics, std::map<int,int> &map_pos, std::vector<int> &cam_ids) {
  ...
  try {
    ...
    for (rapidjson::SizeType curInt = 0; curInt < extrinsicsJson.Size(); curInt++) {
      ...
      map_pos.insert(std::pair<int,int>(key,curInt));
      cam_ids.push_back(key);
    }
